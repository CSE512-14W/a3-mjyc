...
NAV2D.OccupancyGridClientNav = function(options) {
  ...
  client.on('change', function() {
    ...
    if (!that.real) {
      that.viewer.scaleToDimensions(9.5, 9.5);
      that.viewer.shift(client.currentGrid.pose.position.x+1, client.currentGrid.pose.position.y);
      // that.viewer.scaleToDimensions(client.currentGrid.width, client.currentGrid.height);
      // that.viewer.shift(client.currentGrid.x,client.currentGrid.y);
    } else {
      // that.viewer.scaleToDimensions(client.currentGrid.width, client.currentGrid.height);
      that.viewer.scaleToDimensions(550, 550);
      that.viewer.scene.x = -746;
      that.viewer.scene.y = 1452;
    }
